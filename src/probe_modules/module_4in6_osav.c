...
static tunnel_sav_profile_t profile = {
	...
	.proto = TUN_SAV_PROTO_4IN6,
	.outer_ipv6 = true,
	.inner_ipv6 = false,
	.module_name = "4in6_osav",
	.module_desc = "4in6 osav source-address validation scan module",
};

probe_module_t module_4in6_osav;

static int global_initialize(struct state_conf *conf)
{
	return tunnel_sav_common_global_initialize(&profile, &module_4in6_osav, conf);
}
...
probe_module_t module_4in6_osav = {
	...
	.helptext = "4in6 osav SAV scanning module. IPv6 target file supports csv rows 'ipv4,ipv6' (optional header: ipv4,ipv6). Output includes original_target/icmp_type and payload_outer_dst4/payload_outer_dst6 extracted from received payload for reliable matching. Optional --probe-args inner_dst4=,inner_dst6=,inner_src4=,inner_src6=",
};
