...
static tunnel_sav_profile_t profile = {
	...
	.proto = TUN_SAV_PROTO_IPIP,
	.outer_ipv6 = false,
	.inner_ipv6 = false,
	.module_name = "ipip_osav",
	.module_desc = "ipip osav source-address validation scan module",
};

probe_module_t module_ipip_osav;

static int global_initialize(struct state_conf *conf)
{
	return tunnel_sav_common_global_initialize(&profile, &module_ipip_osav, conf);
}
...
probe_module_t module_ipip_osav = {
	...
	.helptext = "ipip osav SAV scanning module. Output includes original_target/icmp_type and payload_outer_dst4/payload_outer_dst6 extracted from received payload for reliable matching. Optional --probe-args inner_dst4=,inner_dst6=,inner_src4=,inner_src6=",
};
